...
Aggregates all run records in outputs/results/runs/ into:
  ...
  - a data-budget sweep figure per axis (metric vs #training points, one line
    per model, error bars = std over seeds),
  - a tidy CSV of every aggregated cell.
These are the paper's results-section artifacts.
...
COLORS = {"Vanilla_MLP": "#888888", "Lagrangian_NN": "#1f77b4",
          "Hamiltonian_NN": "#2ca02c", "Vanilla_GP": "#9467bd",
...
def main():
    ...
    # group[(env, model, n_train)] = list of per-seed run dicts
    group = defaultdict(list)
    for r in runs:
        group[(r["env"], r["model"], r["n_train"])].append(r)
    ...
    with open(res_dir / "summary.csv", "w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["env", "model", "n_train", "n_seeds"]
                   + [f"{k}_mean" for k in metric_keys] + [f"{k}_std" for k in metric_keys])
        for (env, model, n), rs in sorted(group.items()):
            means = [np.mean([r[k] for r in rs]) for k in metric_keys]
            stds = [np.std([r[k] for r in rs]) for k in metric_keys]
            w.writerow([env, model, n, len(rs)] + means + stds)

    for env in envs:
        models = sorted({m for (e, m, _) in group if e == env})
        budgets = sorted({n for (e, _, n) in group if e == env})
        full = budgets[-1]

        # ---- comparison table at the largest budget ----
        print(f"\n### {env}  (largest budget n_train={full}, mean +/- std over seeds)\n")
        headers = ["Model"] + [name for _, name, _ in AXES] + [EXTRA[1]]
        print("| " + " | ".join(headers) + " |")
        print("|" + "|".join(["---"] * len(headers)) + "|")
        for model in models:
            rs = group.get((env, model, full), [])
            if not rs:
                continue
            cells = [model]
            for k, _, _ in AXES:
        ...
        if len(budgets) > 1:
            ...
            for ax, (k, name, higher) in zip(axes, AXES):
                ...
                ax.legend()
            fig.suptitle(f"Data-budget sweep on {env}")
            fig.tight_layout()
